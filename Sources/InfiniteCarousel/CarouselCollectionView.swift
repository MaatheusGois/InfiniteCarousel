...
@objc public protocol CarouselCollectionViewDataSource {
    ...
    @objc optional func carouselCollectionView(
        _ carouselCollectionView: CarouselCollectionView,
...
public class CarouselCollectionView: UICollectionView {
    ...
    private func scrollToNextElement() {
        guard fakeNumberOfItems > .zero else { return }
        if fakeCurrentPage == fakeNumberOfItems - 1 {
            setFakePage(1)
            let newPageRealIndex = fakeCurrentPage < (numberOfItems) ? fakeCurrentPage : .zero
